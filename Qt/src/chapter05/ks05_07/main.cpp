...
int main(int argc, char * argv[])
{
	...
	strPath += "/system/lang";					// $TRAINDEVHOME/system/lang/ks05_07.qm
	QScopedPointer<QTranslator> gpTranslator(new QTranslator(QCoreApplication::instance()));
	if (gpTranslator->load("ks05_07.qm", strPath))
	{
		QCoreApplication::installTranslator(gpTranslator.take());
	}
	CScanThread* pThread = new CScanThread();

	CDialog dlg(NULL);
	pThread->setDialog(&dlg);
	pThread->start();
	dlg.exec();
