...
void CMainWindow::notify(const SLog& log)
{
	QMutexLocker mtx(&m_mtx);
	if (NULL != m_pLogDockWidget)
	{
		CLogEvt* pLogEvt = new CLogEvt(ELogEvt_LogOut);
		if (NULL != pLogEvt)
		{
			pLogEvt->setLog(log);
			QApplication::postEvent(m_pLogDockWidget, pLogEvt);
		}
	}
}
void CMainWindow::createDockWindows_Down()
{
	m_pLogDockWidget = new CLogDockWidget(QString::fromLocal8Bit("日志输出窗口"), this);
	addDockWidget(Qt::BottomDockWidgetArea, m_pLogDockWidget);
}
...
bool CMainWindow::openFile(const QString& str)
{
	SLog log;
	...
	if (!str.isEmpty()) {
		...
		QString str = m_pTextEdit->currentFileName();
		if (str == fileName){
			log.level = ELogLevel_Warning;
			log.msg = QString("file: %1 already open!").arg(str);
			notify(log);
		...
		if (m_pTextEdit->openFile(fileName)) {
			...
			log.level = ELogLevel_Error;
			log.msg = QString("file: %1 can not open!").arg(fileName);
			notify(log);

			return false;
		}
	}
	log.level = ELogLevel_Normal;
	log.time = QTime::currentTime();
	log.msg = QString("file: %1 opened!").arg(str);
	notify(log);
	return true;
}